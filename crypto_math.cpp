...
// Задание 6. Разложение в цепную дробь и проверка уравнения
void fraction(int num, int den) {
    cout << "[Цепные дроби] Разложение " << num << "/" << den << ":\n";
    
    int a = num, b = den;
    vector<int> q;
    
    // Цикл нахождения неполных частных (элементов цепной дроби)
    while (b != 0) {
        q.push_back(a / b);
        cout << "  " << a << " / " << b << " = " << a / b << " (остаток " << a % b << ")\n";
        int r = a % b;
        a = b; 
        b = r;
    }
    
    cout << "  Элементы цепной дроби: [";
    ...
    int dummy_u, dummy_v;
    int g = extendedgcd(num, den, dummy_u, dummy_v);
    
    cout << "  НОД(" << num << ", " << den << ") = " << g << "\n";
    cout << "  Проверка уравнения 1256a + 847b = 119:\n";
    
    if (119 % g != 0) {
        cout << "  Результат: решений в целых числах нет, так как 119 не делится на НОД(" << g << ").\n";
    } else {
        cout << "  Результат: уравнение разрешимо в целых числах, так как 119 делится на НОД(" << g << ").\n";
    }
}
